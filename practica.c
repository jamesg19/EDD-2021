...
void abrirEscrituraArchivoP();
void cerrarEscrituraArchivo();
void cerrarEscrituraArchivoP();
int cantPilCarreta1;
int cantPilCarreta2;
...
void iniciarSimulacion(){
	...
	if( HayClientesEsperando(esperanCarreta)&&HayCarretasenpila1(pilaCarreta1)){
		...
		numeroRandom1 = rand () % ((idClienTot+1)-1+1) + 1;   // Este está entre M y N
		//numeroRandom2 = rand () % ((idClienTot+1)-1+1) + 1;   // Este está entre M y N
		eliminarCompra(compras,numeroRandom1,listaPay);
		//eliminarCompra(compras,numeroRandom2,listaPay);
		
	if(HayCajaDisponible(cajeras)==true){
		printf("\nCAAJAAAAAAAAAA DISPONIBLE DICEEEE: %d",cDispo);
		eliminarListaPago(listaPay,cajeras);
		
		
	}else{
	printf("\nCAAJAAAAAAAAAA nooooo DICEEEE: %d",cDispo);
	int numRandomSalidaCaja;
	numRandomSalidaCaja = rand () % ((cantCaja+3)-1+1) + 1;   // Este está entre M y N
	modificarCajaS(cajeras, numRandomSalidaCaja,pilaCarreta1);
	
	}
	if(bandera==1){ 
	abrirEscrituraArchivoP(pasos);
	mostarColaEspCarreta(esperanCarreta);
	mostarPilaCarreta1(pilaCarreta1,1);
	mostarPilaCarreta2(pilaCarreta2,2);
	desplegarListaCompra();
...
void mostarColaEspCarreta(colaEspCarreta* actual1){
	...
	fprintf(archivo," Cola_de_espera_de_Carreta->Vacio");
		//printf("\n La cola no existe\n\n");
	}
}
...
void insertarPilaCarretaS(PilaCarreta* nuevo, int id,int NoPila){
...
void mostarPilaCarreta1(PilaCarreta* actuall, int n){
	...
	fprintf(archivo," Pila_Carreta1 ->Vaciaa ;  \n");
		//printf("\n La cola no existe\n\n");
	}
}
void mostarPilaCarreta2(PilaCarreta2* actuall, int n){
	...
	if(primeroPilCar2 != NULL){
		fprintf(archivo," Pila_Carreta2%d-> Carreta%d; \n",n,actuall->idCarreta2);
		while(actuall != NULL){
		...
		fprintf(archivo," Pila_Carreta2 -> Vacia ; ID=8 \n");
		//printf("\n La cola no existe\n\n");
	}
}
////////////////////////////////////////////////////////////////////
...
void desplegarListaPago(){
	...
	if(primeroListaPago != NULL){
		...
		fprintf(archivo," Cola_Pago-> Sin__Clientes; ID=7;\n");
		//printf("\n La cola no existe\n\n");
	}
}
/////////////////CAJA/////////////////////////
...
void modificarCaja(Caja* actual, int cDispoo,int C,int K){
	actual = (Caja*) malloc(sizeof(Caja));
	actual = primeroCaja;
	...
	if(primeroCaja!=NULL){
		do{
			if(actual->IDCaja == cDispo && actual->estado ==true){
...
void modificarCajaS(Caja* actual, int n,PilaCarreta* pilaCarretaUno){
	actual = (Caja*) malloc(sizeof(Caja));
	actual = primeroCaja;
	int cajaBuscado = 0, encontrado = 0;
	...
	if(primeroCaja!=NULL){
		do{
			if(actual->IDCaja == n){
				...
				printf("\nEl CLIENTE %d con CARRETA %d  sale de la CAJA -----> Saliendo...%d",actual->idClienteEnCaja,actual->idCarretaEnCaja,actual->IDCaja);
				insertarPilaCarretaS(pilaCarretaUno, actual->idCarretaEnCaja,1);
				actual->estado=true;
...
void abrirEscrituraArchivoP(int p){
	archivo=fopen("Paso.txt","w");
	fprintf(archivo,"digraph james{");
	fprintf(archivo," node [shape= record fontsize=11 ] ");
}
void cerrarEscrituraArchivoP(){
	fprintf(archivo,"}");
	fclose(archivo);
	system("dot -Tpng Paso.txt -o Paso.png");
